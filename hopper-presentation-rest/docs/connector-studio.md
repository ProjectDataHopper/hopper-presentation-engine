...
Contracts are defined with hopper-presentation-core; see also `hopper-presentation-core/docs/connectors.md` (section *Connector studio preview*).
...
5. **Chain**: visual pipeline builder (left connector palette, top icon strip with names, step properties below). Drag types onto the chain; reorder by dragging steps; **Virtual path** at the top is shared for the chain package. Column pickers on later steps (Filter, Select, Aggregate, …) use the **output of previous steps** via `POST edit/connector/describe-inline/` on a partial chain (not the outer source alone).
...
```
POST /hopper/api/edit/connector/preview/
...
```json
{
  "hopperConnectorJson": "{ ... full Hop connector JSON ... }",
  "maxRows": 20,
...
|-------|----------|--------|
| `hopperConnectorJson` | yes | Same shape as `metadata/modify/connector` payload / `connector-json` load |
...
| Endpoint | Role |
|----------|------|
| `POST edit/connector/preview/` | **Studio preview** — input/output samples from inline `hopperConnectorJson` |
...
curl -sS -X POST -H 'Content-Type: application/json' \
  -d '{"maxRows":5,"hopperConnectorJson":"{\"name\":\"Sample Data\",\"connector\":{\"SampleDataConnector\":{\"pluginId\":\"SampleDataConnector\",\"rowCount\":10}}}"}' \
...
| **1** | `@HWidgetElement` completeness (done) |
